...
// 分类绘制蚁群路线图像
void lujinghuoqu(const std::pair<std::vector<Point>, std::vector<Point>>& class_Points, const std::vector<int>& BestTour_forall_1, const std::vector<int>& BestTour_forall_2, int mayi_num)
{
   ...
   // std::cout << diedai_num*50<< std::endl;
    ...
    // 将路径长度写入文件
    outfile << mayi_num*10 << "," <<Tourlength_1 << "," << Tourlength_2 << std::endl;
...
int main() 
{
    ...
    for(int mayi_num = 1; mayi_num <= 10; mayi_num++)
    {
      //  for(int diedai_num = 1; diedai_num<= 10; diedai_num++)
       // {
        // 设置第一类初始参数
        int numAnts_1 = mayi_num*10, maxIterations_1 = 1000; // 蚂蚁数量，最大迭代次数
        double alpha_1 = 1, beta_1 = 1, Q_1 = 100, evaporationRate_1 = 0.7; //信息素影响因子alpha，启发式影响因子beta，信息素挥发速率evaporationRate
        // 设置第二类初始参数
        int numAnts_2 = mayi_num*10, maxIterations_2 = 1000; 
        double alpha_2 = 1, beta_2 = 1, Q_2 = 100, evaporationRate_2 = 0.7;
        ...
        //Mat img_lujinghuoqu =
         lujinghuoqu(classPoints, BestTour_forall_1, BestTour_forall_2, mayi_num);
     //   }
    }
    // 显示图像
