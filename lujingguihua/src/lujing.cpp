...
Mat lujinghuoqu(const std::pair<std::vector<Point>, std::vector<Point>>& class_Points, const std::vector<int>& BestTour_forall_1, const std::vector<int>& BestTour_forall_2, const Mat& img)
{
    ...
    // 输出第二条最佳路径长度
    std::cout << "Tour length_1: " << Tourlength_2 << std::endl;
...
int main() 
{
    ...
    // 执行蚁群算法进行路径规划
    std::vector<int> BestTour_forall_1 = antColonyOptimization(PointsClass1, numAnts_1, maxIterations_1, alpha_1, beta_1, evaporationRate_1, Q_1, generator);
    std::vector<int> BestTour_forall_2 = antColonyOptimization(PointsClass2, numAnts_2, maxIterations_2, alpha_2, beta_2, evaporationRate_2, Q_2, generator);
    // // 输出最佳路径
    // std::cout << "Best tour: ";
    ...
    // 存储图像
    cv::imwrite("../image/Best_Tour.png", img_lujinghuoqu);
